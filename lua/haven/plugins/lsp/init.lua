...
local function on_attach(client, bufnr)
  ...
  keymap("gra", function() fzf().lsp_code_actions({ silent = true }) end, "Code Actions", { "n", "x" })

  keymap("grr", "<cmd>FzfLua lsp_references<cr>", "References")
  keymap("gy", "<cmd>FzfLua lsp_typedefs<cr>", "Go to type definition")
  keymap("gI", "<cmd>FzfLua lsp_implementations", "Implementations")
  ...
  vim.api.nvim_create_autocmd("ColorScheme", {
    desc = "Enable document color on colorscheme change",
    buffer = bufnr,
    callback = function() vim.lsp.document_color.enable(true, bufnr) end,
  })

  if client:supports_method(methods.textDocument_definition) then
    keymap("gd", function() fzf().lsp_definitions({ jump1 = true }) end, "Go to definition")
    keymap("gD", function() fzf().lsp_definitions({ jump1 = false }) end, "Peek definition")
  end
  ...
  if client:supports_method(methods.textDocument_signatureHelp) then
    keymap("<C-k>", function()
      ...
      if require("blink.cmp.completion.windows.menu").win:is_open() then require("blink.cmp").hide() end
  ...
  if client:supports_method(methods.textDocument_documentHighlight) then
    local under_cursor_highlights_group = vim.api.nvim_create_augroup("haven/cursor_highlights", { clear = false })
...
-- -- Set up LSP servers.
-- -- TODO: enable this once I have removed nvim-lspconfig
...
for _, mod in ipairs(mods) do
  spec[#spec + 1] = require("haven.mod.lsp." .. mod)
end
