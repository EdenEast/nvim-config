...
--- Taken from https://github.com/MariaSolOs/dotfiles/blob/02e145c92d89280815071128f6c6eb4a59911730/.config/nvim/lua/lightbulb.lua
...
local lb_name = "haven/lightbulb"
local lb_namespace = vim.api.nvim_create_namespace(lb_name)
local lb_icon = require("haven.icons").diagnostics.HINT
...
local function update(bufnr, client)
  ...
  timer:start(100, 0, function()
    ...
    vim.schedule(function()
      if vim.api.nvim_buf_is_valid(bufnr) and vim.api.nvim_get_current_buf() == bufnr then render(bufnr, client) end
    end)
  end)
end
