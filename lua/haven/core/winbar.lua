-- Taken from https://github.com/MariaSolOs/dotfiles/blob/c40a0b63a1ca0f9ed6d2969f59e7724a9a998861/.config/nvim/lua/winbar.lua
local folder_icon = require("haven.icons").symbol_kinds.Folder
...
function M.render()
  ...
  return table.concat({
    ...
    prefix,
    table.concat(
      vim.iter(vim.split(path, "/")):map(function(segment) return string.format("%%#Winbar#%s", segment) end):totable(),
      separator
    ),
  })
end
...
function M.setup()
  vim.api.nvim_create_autocmd("BufWinEnter", {
    group = vim.api.nvim_create_augroup("haven/winbar", { clear = true }),
    desc = "Attach winbar",
    callback = function(args)
      ...
      then
        vim.wo.winbar = "%{%v:lua.require'haven.core.winbar'.render()%}"
      end
    end,
  })
end
